...
export async function createProfile(data: Partial<Profile>): Promise<void> {
  const { id, email, first_name, last_name, role, phone_number, password_hash } = data;
  await executeAction(
    'INSERT INTO profiles (id, email, first_name, last_name, role, phone_number, password_hash) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [id, email, first_name, last_name, role || 'user', phone_number, password_hash || null]
  );
}
